...
open class GetterMatcher<T>(private val getter: (T) -> Any?, private val expected: Any? = null) :
    ...
    override fun matchesSafely(item: T, mismatchDescription: Description) = (getter.invoke(item)?.equals(expected) ?: (expected == null))
...
fun <T> assertFields(value: T, vararg raw: Any) =
    Assert.assertThat("Actual $value fields are not equal to expected.", value, CoreMatchers.allOf(
        raw.map {
            when (it) {
                is Pair<*, *> -> if (it.first is Getter<*>) GetterMatcher(it.first as (T) -> Any?, it.second)
                else throw RuntimeException("Can't pass pair without getter.")
                is Getter<*> -> GetterMatcher(it as (T) -> Any?)
                else -> throw RuntimeException("Invalid argument type.")
            }
        }
    ))